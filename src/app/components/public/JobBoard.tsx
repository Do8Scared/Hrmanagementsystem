...
// ── HOME ──────────────────────────────────────────────────────────────────────
function HomePage({ setPage, publicJobs }: { setPage: (p: NavPage) => void; publicJobs: JobPosting[] }) {
  return (
    <div>
      {/* Hero */}
...
// ── CAREERS ───────────────────────────────────────────────────────────────────
function CareersPage({ publicJobs }: { publicJobs: JobPosting[] }) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: 'Nunito, system-ui, sans-serif', background: '#fff' }}>
      ...
      <main className="flex-1">
        {page === 'home'     && <HomePage     setPage={setPage} publicJobs={publicJobs} />}
        {page === 'about'    && <AboutPage />}
        {page === 'services' && <ServicesPage setPage={setPage} />}
        {page === 'careers'  && <CareersPage publicJobs={publicJobs} />}
