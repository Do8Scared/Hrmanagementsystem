document.addEventListener('DOMContentLoaded', () => {
  ...
  // NTE and NOD records for the manager's team
  let ntes = [
    { id: 'NTE-1718500000', employeeId: 'EMP002', employeeName: 'Juan dela Cruz', department: 'Engineering', issuedBy: 'Maria Santos', issuedByRole: 'manager', assignedTo: 'Sofia Garcia', issuedDate: '2026-06-16', incidentDate: '2026-06-15', incidentType: 'Habitual Tardiness', description: 'Late for 5 consecutive days without prior notice.', responseDeadline: '2026-06-21', status: 'Pending Explanation', escalationRequired: false, auditLog: [{ id: 'AL-1', action: 'NTE Issued', performedBy: 'Maria Santos', role: 'manager', timestamp: '2026-06-16, 09:00:00' }] },
    { id: 'NTE-1718400000', employeeId: 'EMP005', employeeName: 'Elena Mendoza', department: 'Operations', issuedBy: 'Maria Santos', issuedByRole: 'manager', assignedTo: 'Sofia Garcia', issuedDate: '2026-06-15', incidentDate: '2026-06-14', incidentType: 'Misconduct', description: 'Altercation with a coworker.', responseDeadline: '2026-06-20', status: 'Explanation Submitted', escalationRequired: false, explanationLetter: { filename: 'Explanation_Mendoza.pdf', submittedDate: '2026-06-18', submittedBy: 'Elena Mendoza' }, auditLog: [{ id: 'AL-2', action: 'NTE Issued', performedBy: 'Maria Santos', role: 'manager', timestamp: '2026-06-15, 10:00:00' }, { id: 'AL-3', action: 'Explanation Submitted', performedBy: 'Elena Mendoza', role: 'employee', timestamp: '2026-06-18, 14:00:00' }] },
  ...
  // Employees under the manager's supervision
  const employees = [
  ...
  // Status and decision badge styles
  const statusCfg = {
  ...
  // DOM references — cases table, search, filter, tabs, and modals
  ...
  // Populate employee and incident type dropdowns
  nteEmployee.innerHTML = '<option value="">Select employee...</option>' + employees.map(e => `<option value="${e.id}">${e.name} &mdash; ${e.department}</option>`).join('');
  ...
  // Cases tab — renders NTE rows filtered by search text and status
  function renderCases() {
    ...
    filtered.forEach(n => {
      ...
      tr.innerHTML = `
        ...
        <td class="px-4 py-3"><div class="text-sm font-semibold text-foreground">${n.employeeName}</div><div class="text-[10px] text-muted-foreground uppercase tracking-wider">${n.department}</div></td>
        ...
        <td class="px-4 py-3"><button class="view-case-btn px-3 py-1.5 rounded-lg text-xs font-bold border border-border text-foreground hover:bg-secondary transition-colors" data-id="${n.id}">View Details</button></td>
      `;
  ...
  // Reports tab — case count summary and department breakdown
  function renderReports() {
    ...
    container.innerHTML = `
      <div class="grid grid-cols-3 gap-4">
        <div class="bg-card rounded-xl border border-border p-4"><div class="text-2xl font-bold text-foreground">${total}</div><div class="text-xs text-muted-foreground mt-0.5">Total Cases</div></div>
        <div class="bg-card rounded-xl border border-border p-4"><div class="text-2xl font-bold text-amber-700">${active}</div><div class="text-xs text-muted-foreground mt-0.5">Active Cases</div></div>
        <div class="bg-card rounded-xl border border-border p-4"><div class="text-2xl font-bold text-purple-700">${nodCount}</div><div class="text-xs text-muted-foreground mt-0.5">NODs Issued</div></div>
      </div>
      <div class="bg-secondary/20 p-5 rounded-xl border border-border mt-5">
        <h3 class="font-bold text-sm mb-3">Cases by Department</h3>
        <div class="space-y-2">${Object.entries(ntes.reduce((acc, curr) => { acc[curr.department] = (acc[curr.department] || 0) + 1; return acc; }, {})).map(([d, c]) => `<div class="flex items-center justify-between text-sm"><span class="text-muted-foreground">${d}</span><span class="font-bold text-foreground">${c}</span></div>`).join('')}</div>
      </div>
    `;
  }

  // Templates tab — lists NTE and NOD letter templates
  ...
  // Detail modal — NTE metadata, explanation letter, NOD block, and status action buttons
  function openDetailModal(id) {
    ...
    if (nte.status !== 'Voided' && nte.status !== 'Closed') {
      let btns = '';
      if (nte.status === 'Explanation Submitted' && !nod) btns += `<button id="mark-review-btn" class="px-4 py-2 rounded-lg bg-purple-600 text-white text-sm font-bold hover:bg-purple-700">Mark Under Review</button>`;
      if (nte.status === 'Under Review' && !nod) btns += `<button id="issue-nod-open-btn" class="px-4 py-2 rounded-lg bg-primary text-white text-sm font-bold hover:bg-primary/90">Issue NOD</button>`;
    ...
    detailModalContent.innerHTML = `
      <div class="flex items-center justify-between px-6 py-5 border-b border-border sticky top-0 bg-white">
        <div>
          <div class="flex items-center gap-2 mb-1"><span class="font-black text-lg text-foreground">${nte.id}</span><span class="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-semibold ${c.bg} ${c.text}"><span class="w-1.5 h-1.5 rounded-full ${c.dot}"></span>${nte.status}</span></div>
          <div class="text-xs text-muted-foreground">${nte.employeeName} &middot; ${nte.department}</div>
        </div>
      ...
      <div class="p-6 space-y-5">
        ...
        <div><div class="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1.5">Description</div><div class="bg-secondary/20 border border-border rounded-xl p-4 text-sm text-foreground leading-relaxed">${nte.description}</div></div>
        ${nte.explanationLetter ? `<div class="p-4 border border-emerald-200 bg-emerald-50 rounded-xl"><div class="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-2">Explanation Submitted</div><div class="text-sm text-emerald-900 flex items-center gap-2"><i data-lucide="file-text" style="width:16px; height:16px;"></i> ${nte.explanationLetter.filename} (on ${nte.explanationLetter.submittedDate})</div></div>` : ''}
        ${nod ? `<div class="p-4 border border-purple-200 bg-purple-50 rounded-xl"><div class="flex items-center justify-between mb-2"><div class="text-xs font-bold text-purple-800 uppercase tracking-wider">Notice of Decision</div><span class="text-[10px] font-bold px-2 py-0.5 rounded-full ${decisionCfg[nod.decision]} uppercase tracking-wider">${nod.decision}</span></div><div class="text-sm text-purple-900 leading-relaxed">${nod.details}</div></div>` : ''}
      </div>
      ${actionsHtml}
    `;
    ...
    if (markBtn) markBtn.addEventListener('click', () => { nte.status = 'Under Review'; detailModal.classList.add('hidden'); renderCases(); });
    const nodBtn = document.getElementById('issue-nod-open-btn');
    if (nodBtn) nodBtn.addEventListener('click', () => { detailModal.classList.add('hidden'); document.getElementById('nod-nte-id').value = id; issueNodModal.classList.remove('hidden'); });
    const voidBtn = document.getElementById('void-btn');
    if (voidBtn) voidBtn.addEventListener('click', () => { if (confirm('Void this NTE?')) { nte.status = 'Voided'; detailModal.classList.add('hidden'); renderCases(); } });
  }

  // Tab switching — Cases, Reports, Templates
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      tabs.forEach(t => { t.className = 'tab-btn px-4 py-2 rounded-lg text-sm font-semibold transition-colors text-muted-foreground hover:text-foreground hover:bg-secondary'; });
  ...
  // Cases search and status filter
  searchInput.addEventListener('input', renderCases);
  filterStatus.addEventListener('change', renderCases);

  // Issue NTE button — opens the NTE creation modal
  document.getElementById('issue-nte-btn').addEventListener('click', () => issueNteModal.classList.remove('hidden'));

  // Issue NTE form — creates a new NTE attributed to Maria Santos and prepends it to the list
  issueNteForm.addEventListener('submit', e => {
    e.preventDefault();
    const emp = employees.find(x => x.id === document.getElementById('nte-employee').value);
    ntes.unshift({
  ...
  // NOD decision buttons — toggle active style and show suspension days when selected
  nodDecisionBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const activeClass = val === 'Exonerated' ? 'border-emerald-500 bg-emerald-50 text-emerald-800'
        : val === 'Dismissal'  ? 'border-red-500    bg-red-50    text-red-800'
        : val === 'Suspension' ? 'border-orange-500 bg-orange-50 text-orange-800'
  ...
  // Issue NOD form — creates a NOD and advances the linked NTE to Decision Issued
  ...
  // Modal close buttons — closes the nearest fixed ancestor overlay
